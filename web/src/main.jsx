...
function App() {
  ...
  const selectedValue = useMemo(() => { if (!document || !selected) return null; if (selected.kind === "message") return document.messages.find(item => item.id === selected.id); return document.messages.flatMap(item => item.parts).find(item => item.id === selected.id) }, [document, selected])
  ...
  useEffect(() => { setSessionTitle(document?.session?.title || "") }, [document?.session?.title])
  useEffect(() => { setEditor(selectedValue ? pretty(selectedValue.data) : "") }, [selectedValue])
  const loadSession = async id => { const body = await request(`/api/session?id=${encodeURIComponent(id)}`); setSessionId(id); setDocument(body); setEditing(false); setSelected(null); setMessageSearch(""); requestAnimationFrame(() => document.querySelector(".timeline")?.scrollTo({ top: document.querySelector(".timeline")?.scrollHeight })) }
  const startEditing = async () => { const body = await request("/api/workspace", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId }) }); setDocument(body.document); setEditing(true); setStatus("Workspace started") }
  const change = async (operation, payload) => { try { const body = await request("/api/workspace/change", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId, operation, ...payload }) }); setDocument(body.document); setStatus("Workspace updated") } catch (error) { setStatus(error.message) } }
  ...
  return <div className="app">
    ...
    <main className="main"><div className="toolbar"><h2>{document?.session?.title || "Select a session"}</h2><span className={`mode ${editing ? "" : "source"}`}>{editing ? "EDITING" : "BROWSE"}</span><button className="button" disabled={!document || editing} onClick={startEditing}>Start editing</button><button className="button primary" disabled={!editing} onClick={apply}>Apply</button><button className="button" disabled={!editing}>Undo</button><button className="button" disabled={!editing}>Redo</button><button className="button" onClick={() => setInspectorOpen(value => !value)}>Inspector</button></div><Timeline document={document} selected={selected} onSelect={(kind, id) => { setSelected({ kind, id }); setInspectorOpen(true) }} searchQuery={messageSearch} onSearchChange={value => { setMessageSearch(value); setMatchIndex(0) }} onSearchMove={matchMove} /></main>
